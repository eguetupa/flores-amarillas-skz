...
/* ==============================================================================
   5. REPRODUCTOR DE MÚSICA CON AUDIO REAL (STRAY KIDS - YOUTIFUL) & SINTETIZADOR
   ============================================================================== */
function initMusicPlayer() {
  ...
  const simulatedDuration = 186; // 3m 06s
  ...
  const playlist = [
    ...
    {
      title: 'Connected (Yellow Flowers Lofi)',
      artist: 'Bang Chan (SKZ-RECORD)',
      quote: '"We’re always connected, no matter what 🐺💛"',
      isAudioFile: false
    },
    {
      title: 'Hug Me (Wolf Chan Cozy Room)',
      artist: 'Bang Chan feat. Wolf Chan',
      quote: '"A warm safe place whenever you need comfort 🐾"',
      isAudioFile: false
    }
  ];
  ...
  // Si hay elemento de audio, cargar duración inicial cuando esté listo
  if (audioEl) {
    audioEl.addEventListener('loadedmetadata', () => {
      if (playlist[currentTrackIdx].isAudioFile && totalTimeEl) {
        totalTimeEl.innerText = formatTime(audioEl.duration);
      }
    });

    audioEl.addEventListener('timeupdate', () => {
      if (!playlist[currentTrackIdx].isAudioFile) return;
  ...
  // Adelantar o retroceder haciendo clic en la barra de progreso
  if (progressContainer) {
    progressContainer.addEventListener('click', (e) => {
      ...
      if (playlist[currentTrackIdx].isAudioFile && audioEl && audioEl.duration) {
        audioEl.currentTime = ratio * audioEl.duration;
      } else {
        simulatedSeconds = ratio * simulatedDuration;
        updateSimulatedTimer();
      }
    });
  }

  // Acordes Lo-fi con Web Audio API de respaldo (Cmaj9, Am9, Fmaj7, Gsus4)
  ...
  function startPlayback() {
    ...
    if (track.isAudioFile && audioEl) {
      if (synthInterval) clearInterval(synthInterval);
      audioEl.muted = isMuted;
      audioEl.play().catch(() => {
        // En caso de bloqueo por política de autoplay o archivo no disponible, recurrir a sintetizador
        fallbackToSynth();
      });
    } else {
      fallbackToSynth();
  ...
  function fallbackToSynth() {
    ...
    synthInterval = setInterval(() => {
      playSynthChord();
      simulatedSeconds += 2;
      if (simulatedSeconds >= simulatedDuration) simulatedSeconds = 0;
      updateSimulatedTimer();
    }, 2400);
  }
  ...
  function updateSimulatedTimer() {
    if (currentTimeEl) currentTimeEl.innerText = formatTime(simulatedSeconds);
    if (totalTimeEl) totalTimeEl.innerText = formatTime(simulatedDuration);
    if (progressBar) {
      const percent = (simulatedSeconds / simulatedDuration) * 100;
      progressBar.style.width = `${percent}%`;
    }
  }
